...
def test_advanced_actions_attribute_access_emits_deprecation_warning():
    with pytest.warns(
        DeprecationWarning,
        match=r"application\.tag_actions\.create_tag_action",
    ):
        action = advanced_actions.create_tag_action
...
async def test_create_tag_action_refreshes_tag_list_after_success():
    with patch(
        "application.tag_actions.tag_service.create_tag",
        new=AsyncMock(return_value={"status": "success"}),
    ) as create_mock, patch(
        "application.tag_actions.tag_service.get_all_tags",
...
async def test_create_subtask_action_refreshes_subtasks_after_success():
    with patch(
        "application.subtask_actions.subtask_service.create_subtask",
        new=AsyncMock(return_value={"status": "success"}),
    ) as create_mock, patch(
        "application.subtask_actions.subtask_service.get_subtasks",
        new=AsyncMock(return_value=[{"subtask_id": "sub_1", "name": "拆解"}]),
    ) as list_mock:
        with pytest.warns(
            DeprecationWarning,
            match=r"application\.subtask_actions\.create_subtask_action",
        ):
...
async def test_start_pomodoro_action_returns_active_session():
    with patch(
        "application.pomodoro_actions.pomodoro_service.start_pomodoro",
        new=AsyncMock(return_value={"status": "success"}),
    ) as start_mock, patch(
        "application.pomodoro_actions.pomodoro_service.get_active_pomodoro",
        new=AsyncMock(return_value={"session_id": "p1"}),
    ) as active_mock:
        with pytest.warns(
            DeprecationWarning,
            match=r"application\.pomodoro_actions\.start_pomodoro_action",
        ):
...
async def test_batch_update_tasks_action_normalizes_optional_tag_lists():
    with patch(
        "application.task_detail_actions.task_command_service.batch_update_tasks",
        new=AsyncMock(return_value={"status": "success", "updated": 2}),
    ) as mocked:
        with pytest.warns(
            DeprecationWarning,
            match=r"application\.task_detail_actions\.batch_update_tasks_action",
        ):
...
async def test_get_task_detail_action_delegates_to_task_detail_service():
    with patch(
        "application.task_detail_actions.task_detail_service.get_task_detail",
        new=AsyncMock(return_value={"status": "success", "task": {"task_id": "task_1"}}),
    ) as mocked:
        with pytest.warns(
            DeprecationWarning,
            match=r"application\.task_detail_actions\.get_task_detail_action",
        ):
