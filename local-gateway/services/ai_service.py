        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["add_task", "delete_task", "get_weekly_plan", "complete_task", "get_pending_tasks"],
                        "description": "操作类型",
                    },
                    ...
                    "start_time": {"type": "string", "description": "任务开始时间，ISO 8601 格式，如 2026-05-10T09:00:00+08:00"},
                    "end_time": {"type": "string", "description": "任务结束时间，ISO 8601 格式，如 2026-05-10T11:00:00+08:00"},
                    "recurrence": {
                        "type": "string",
                    ...
                    "tasks": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "start_time": {"type": "string", "description": "任务开始时间，ISO 8601"},
                                "end_time": {"type": "string", "description": "任务结束时间，ISO 8601"},
...
def _get_system_prompt() -> str:
    """返回包含当前日期的 system prompt"""
    from datetime import datetime
    today = datetime.now().strftime("%Y年%m月%d日")
    weekday_map = {0: "星期一", 1: "星期二", 2: "星期三", 3: "星期四", 4: "星期五", 5: "星期六", 6: "星期日"}
    weekday = weekday_map[datetime.now().weekday()]
    date_line = f"\n今天是 {today}（{weekday}）。涉及日期/时间时以此为准，不要编造日期。\n"
    return SYSTEM_PROMPT_BASE + date_line

# System prompt（不含日期，由 _get_system_prompt 动态拼接）
SYSTEM_PROMPT_BASE = """你是 LocalCommandCenter 助手，一个强大的本地系统 Agent。

## 核心能力（8 个工具）

### 基础工具（高频操作，直接调用）
1. **local_task_manager** — 任务管理：添加/删除/完成/查询/未完成任务。支持设置执行时间段（start_time/end_time）。
...
7. **先查后建**：创建任务前，先用 get_pending_tasks 或 get_weekly_plan 查看已有安排，避免时间冲突和过载
...
def _save_conversation_message(conversation_id: str, role: str, content: str, thinking: str = "", model: str = ""):
    ...
    # 用户消息且对话尚无标题时，自动设置标题
    if role == "user" and (not meta.get("title") or meta.get("title") == "新对话"):
        if content and content.strip():
            meta["title"] = content.strip()[:30]
        ...
        with open(conv_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if role not in ("user", "assistant", "tool"):
                    logger.warning(f"跳过损坏消息（无效 role={role}）: {str(msg)[:100]}")
                    continue

                # 跳过 assistant 消息中 tool_calls 格式异常的
                if role == "assistant" and msg.get("tool_calls"):
                    valid_tcs = []
                    for tc in msg["tool_calls"]:
                        fn = tc.get("function", {})
                        if not isinstance(fn, dict) or not fn.get("name"):
                            logger.warning(f"跳过损坏的 tool_call: {str(tc)[:100]}")
                            continue
                        valid_tcs.append(tc)
                    if not valid_tcs:
                        logger.warning("assistant 消息的 tool_calls 全部无效，跳过整条消息")
                        continue
...
def _save_conversation_meta(conversation_id: str, title: str = None):
    """保存/更新对话元数据"""
    from config import BASE_DIR
    from datetime import datetime
...
def _load_conversation_meta(conversation_id: str) -> dict:
    """加载对话元数据，不存在时返回空结构"""
    from config import BASE_DIR

    meta_file = BASE_DIR / "data" / "conversations" / f"{conversation_id}_meta.json"
    empty = {"title": "", "created_at": "", "updated_at": "", "message_count": 0}
