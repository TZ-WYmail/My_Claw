...
SHELL_SAFE_COMMANDS = {
    ...
    "echo", "pwd", "whoami", "date", "uname", "which", "file", "stat",
    "tree", "diff", "sort", "uniq", "cut", "tr", "wc", "tee",
}
SHELL_DANGEROUS_PATTERNS = ["rm -rf /", "sudo rm", "mkfs", "dd if=", "> /dev/sd", "chmod 777 /"]
    ...
    # ==================== 新增：批量任务编排 ====================
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "此工具会自动将用户的自然语言任务列表解析为结构化数据：识别任务名称和截止日期，"
...
- 按时间线排列，发现日期冲突和过载问题
- 提出执行建议：哪些可以并行、哪些有前置依赖

### 角色 B：反思者 🤔
- 以用户视角审视规划者的方案
- 指出问题：时间太紧、任务太密集、遗忘的细节、不切实际的安排
- 提出改进：拆分大任务、调整时间、合并同类项、标注优先级
- 模拟用户可能担心的问题
...
async def chat(user_message: str, conversation_id: str = "default") -> dict:
    ...
    try:
        ...
        for step in range(15):
            ...
            logger.info(f"Agent step {step+1}: {len(message['tool_calls'])} tool call(s)")
