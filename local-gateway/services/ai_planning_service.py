...
from services import ai_planning_replan_service
from services import ai_planning_variant_service
...
def _select_variant_plan(preview: dict, selected_variant: str) -> dict:
    return ai_planning_preview_service.select_variant_plan(preview, selected_variant)


def _task_schedule_from_variant(task: dict, variant_plan: dict) -> dict:
    return ai_planning_preview_service.task_schedule_from_variant(task, variant_plan)


def _topological_sort_tasks(tasks: list[dict]) -> tuple[list[dict], list[dict]]:
    return ai_planning_preview_service.topological_sort_tasks(tasks)


def _extract_conflict_chain(variant_plan: dict) -> list[dict]:
    return ai_planning_preview_service.extract_conflict_chain(variant_plan)


def _build_replan_context(tasks: list[dict], preview: dict, interrupt_task: dict | None = None) -> dict:
    return ai_planning_preview_service.build_replan_context(tasks, preview, interrupt_task)


async def preview_task_plan(tasks: list[dict], constraints: dict | None = None) -> dict:
    return await ai_planning_preview_service.preview_task_plan(
        tasks=tasks,
        constraints=constraints,
        build_variant_plan_fn=ai_planning_variant_service.build_variant_plan,
    )


async def confirm_task_plan(preview_id: str, selected_variant: str = "balanced", user_adjustments: dict | None = None) -> dict:
    preview, variant_plan, tasks = await ai_planning_preview_service.confirm_task_plan(
        preview_id=preview_id,
        selected_variant=selected_variant,
        user_adjustments=user_adjustments,
    )
    if preview is None or tasks is None:
        return variant_plan

    create_payload = ai_planning_preview_service.build_create_payload(
        tasks=tasks,
        variant_plan=variant_plan,
        selected_variant=selected_variant,
    )
    created = await task_command_service.batch_add_tasks(create_payload)
    await ai_planning_preview_service.finalize_confirmed_preview(preview_id)
    return ai_planning_preview_service.count_variant_summary(
        tasks=tasks,
        variant_plan=variant_plan,
        selected_variant=selected_variant,
        created=created,
...
async def replan_tasks(tasks: list[dict], constraints: dict | None = None, interrupt_task: dict | None = None) -> dict:
    ...
    llm_replan = await ai_planning_replan_service.llm_reorder_conflict_tasks(replan_context)
    if llm_replan.get("status") != "success":
        llm_replan = ai_planning_replan_service.fallback_reorder_conflict_tasks(replan_context)
    suggested_tasks, applied_actions = ai_planning_replan_service.apply_reorder_suggestions(
        merged_tasks,
        llm_replan.get("reordered_tasks", []),
    )
